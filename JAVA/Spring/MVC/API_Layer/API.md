...
Controller는 클라이언트의 요청을 직접적으로 전달 받는 엔드포인트(End Point)로써 Model과 View 중간에서 상호 작용해주는 역할을 한다. 즉, 클라이언트의 요청을 받아 비즈니스 로직을 거친 후 Model 데이터가 만들어지면, 이 Model 데이터 View로 전달하는 역할을 한다.
